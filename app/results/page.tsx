...
export default function ResultsPage() {
  ...
  const [metrics, setMetrics] = useState({ score: 0, completeness: 0, consistency: 0, accuracy: 0, validity: 0 });
  ...
  useEffect(() => {
    try {
      ...
      setColumns(Object.keys(r[0] ?? {}));
    } catch {
      setRows([]);
      setMetrics({ score: 0, completeness: 0, consistency: 0, accuracy: 0, validity: 0 });
      setColumns([]);
    }
  }, []);
  ...
  columns.forEach((col) => {
    ...
    const isNumeric = values.every((v) => v === "" || v == null || !Number.isNaN(Number(v)));
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 320px", gap: 16, alignItems: "start" }}>
                <div className="card">
                  ...
                  <div className="table-wrapper">
                    {columns.length ? <Bar data={barData} /> : <div className="muted">No data</div>}
                  </div>
                </div>
                <div className="card">
                  <h3>Column types</h3>
                  <div className="table-wrapper">{columns.length ? <Pie data={pieData} /> : <div className="muted">No data</div>}</div>
                </div>
              </div>
            </section>
          </div>
          ...
          <aside>
            <div className="card panel">
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <Link href="/insights">
                  <button className="btn">AI Insights</button>
                </Link>
